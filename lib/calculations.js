 ...
 * If not, boosts fixed_unlinked (or fixed_cpi if present) until the rule is met.
 */
function enforceFixedMinimum(mix) {
  ...
  // Add deficit to fixed_unlinked (add entry if missing)
  ...
  // Reduce deficit proportionally from non-fixed tracks
...
function recommendMix(goals) {
  ...
  if (has("stability") && !has("low_total"))
    mix = [{ track: "fixed_unlinked", pct: 50 }, { track: "fixed_cpi", pct: 33 }, { track: "prime", pct: 17 }];
  else if (has("low_monthly") && !has("stability"))
    // שליש קבועה לא צמודה + שני שליש זולים — עמידה בחוק הריבית הקבועה
    mix = [{ track: "prime", pct: 34 }, { track: "fixed_unlinked", pct: 33 }, { track: "variable_unlinked", pct: 33 }];
  else if (has("early_repay"))
    mix = [{ track: "prime", pct: 33 }, { track: "fixed_unlinked", pct: 34 }, { track: "fixed_cpi", pct: 33 }];
  else if (has("low_total"))
    mix = [{ track: "prime", pct: 33 }, { track: "fixed_unlinked", pct: 34 }, { track: "variable_unlinked", pct: 33 }];
  else
    mix = [{ track: "fixed_unlinked", pct: 33 }, { track: "fixed_cpi", pct: 34 }, { track: "prime", pct: 33 }];
