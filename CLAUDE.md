...
### כלל נוסף — מסלול קבוע יחיד בתמהיל
- **אין לשלב גם `fixed_unlinked` וגם `fixed_cpi` באותו תמהיל** — יש לבחור אחד מהם בלבד
- שני השלישים הנותרים: `prime` בלבד / `variable_unlinked` בלבד / שילוב של השניים
- אין להוסיף `variable_cpi` לתמהיל הרגיל (רק במקרים חריגים מאוד)

### חישוב ריבית פריים — ספרד לפי אחוז בתמהיל
**ריבית הפריים האפקטיבית = ריבית פריים נוכחית + ספרד לפי שיעור הפריים בתמהיל:**

| אחוז פריים בתמהיל | ספרד | דוגמה (פריים 5.5%) |
|---|---|---|
| פחות מ-40% | P - 0.85% | 4.65% |
| 40% עד 50% | P - 0.75% | 4.75% |
| מעל 50% | P - 0.60% | 4.90% |

- מממש ב-`getPrimeSpread(primePct)` ו-`effectivePrimeRate(primeRate, primePct)` ב-`lib/calculations.js`
- `scoreBank()` מקבל `primeRate` מהcache ומחשב ספרד אוטומטית לפי הרכב התמהיל
